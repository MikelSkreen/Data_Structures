...
public:
	...
	bool Add(T& toCopyAndAdd, Node** pCur)
	{
		...
		else if ((*pCur)->C)
		{
			...
			if (toCopyAndAdd.GetKey() < (*pCur)->A->GetKey())
				return Add(toCopyAndAdd, &(*pCur)->L);
			return Add(toCopyAndAdd, &(*pCur)->M1);
		}
		else if ((*pCur)->A)
		{
			if ((*pCur)->B)
				...
				if (toCopyAndAdd.GetKey() > (*pCur)->B->GetKey())
				{
					if ((*pCur)->M2)
					{
						...
						if (toCopyAndAdd.GetKey() > (*pCur)->C->GetKey())
							return Add(toCopyAndAdd, &(*pCur)->R);
						return Add(toCopyAndAdd, &(*pCur)->M2);
					}
					else
					{
						(*pCur)->C = &toCopyAndAdd;
